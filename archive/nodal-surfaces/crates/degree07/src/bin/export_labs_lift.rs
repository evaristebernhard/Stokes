...
fn main() {
    ...
    let support_mask = match parse_args(&args) {
        ...
        Err(message) => {
            eprintln!("{message}");
            eprintln!("usage: export_labs_lift <support-mask 1..15>");
            process::exit(2);
        }
    };
    ...
    print_lift_script(
        &format!("labs_support_{support_mask:02}_lift"),
        stratum.generators(),
        &["u0", "u1", "u2", "loc"],
    );
}
...
fn print_lift_script(
    ...
    generators: &[SparsePolynomial<CubicAlphaRational, LABS_PROJECTIVE_STRATUM_VARIABLE_COUNT>],
    variables: &[&str; LABS_PROJECTIVE_STRATUM_VARIABLE_COUNT],
) {
    println!("ring r = (0,a),({}),dp;", variables.join(","));
    println!("minpoly = 7*a^3+7*a+1;");
    println!("print(\"# generated_by export_labs_lift\");");
...
fn singular_polynomial(
    polynomial: &SparsePolynomial<CubicAlphaRational, LABS_PROJECTIVE_STRATUM_VARIABLE_COUNT>,
    variables: &[&str; LABS_PROJECTIVE_STRATUM_VARIABLE_COUNT],
) -> String {
