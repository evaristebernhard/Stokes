...
type PolynomialA4 = SparsePolynomial<QuadraticRational, BARTH_PROJECTIVE_STRATUM_VARIABLE_COUNT>;
...
fn main() {
    ...
    let support_mask = match parse_args(&args) {
        ...
        Err(message) => {
            eprintln!("{message}");
            eprintln!("usage: export_barth_projective_strata <support-mask 1..15>");
            process::exit(2);
        }
    };
    ...
    println!("print(\"# barth_support_mask {support_mask}\");");
