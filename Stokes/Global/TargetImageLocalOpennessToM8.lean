...
/-!
# Local-openness target-image data as M8 input

This file is the global adapter from the pure boundary-chart
`BoundaryChartLocalOpennessTargetImageFamily` package to the existing M8
target-image input shape.

Local openness is used only to produce the pure resolved target-image family.
The genuinely global data remain explicit: source extended boxes, boundary
partition endpoint boxes, selected partition endpoints, boundary-partition term
identification, and oriented-atlas membership.
-/
...
section TargetImageLocalOpennessToM8
...
from local-openness target-image data.
-/
structure M8TargetImageLocalOpennessInput {n : Nat}
  ...
  /-- Pure local-openness target-image family.  M8 chart labels are manifold points. -/
  family : BoundaryChartLocalOpennessTargetImageFamily I omega M BoundaryPiece
    ...
    forall x, x ∈ family.toTargetImageResolvedFamily.activeCharts ->
      forall q, q ∈ family.toTargetImageResolvedFamily.localPieces x ->
        boundaryChartExtendedBox I
          (family.toTargetImageResolvedFamily.sourceChart x q)
...
/-- The pure resolved target-image family induced by the local-openness data. -/
...
/-- Forget the local-openness source down to the existing resolved-family M8 input. -/
def toResolvedInput
    (D :
      M8TargetImageLocalOpennessInput I omega selectedPartition
        orientedBoundaryAtlas BoundaryPiece) :
...
/-- Local-openness target-image data in the exact shape consumed by M8. -/
...
/-- Direct M8 global input constructor using local-openness target-image data. -/
...
/-- Direct constructor from a pure local-openness family to the M8 resolved wrapper. -/
def toM8ResolvedInput
    (F : BoundaryChartLocalOpennessTargetImageFamily I omega M BoundaryPiece)
    (sourceExtendedBox :
  ...
  F.toTargetImageResolvedFamily.toM8ResolvedInput sourceExtendedBox
    ...
    boundaryPartitionTerm boundaryPartitionTerm_eq active_eq source_mem
    boundarySource_mem boundaryTarget_mem

/-- Direct constructor from a pure local-openness family to `M8TargetImageInput`. -/
def toM8TargetImageInput
    (F : BoundaryChartLocalOpennessTargetImageFamily I omega M BoundaryPiece)
    (sourceExtendedBox :
      forall x, x ∈ F.toTargetImageResolvedFamily.activeCharts ->
...
end TargetImageLocalOpennessToM8
